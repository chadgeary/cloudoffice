...
resource "digitalocean_droplet" "nc-droplet" {
  ...
  user_data                         = "#!/bin/bash\napt-get update\nDEBIAN_FRONTEND=noninteractive apt-get -y install python3-pip git\npip3 install --upgrade ansible\nmkdir /opt/git/nextcloud\ngit clone ${var.project_url} /opt/git/nextcloud/\ncd /opt/git/nextcloud/\ngit pull\ncd playbooks/\nansible-playbook nextcloud_do.yml --extra-vars 'docker_network=${var.docker_network} docker_gw=${var.docker_gw} docker_nextcloud=${var.docker_nextcloud} docker_db=${var.docker_db} docker_webproxy=${var.docker_webproxy} docker_storagegw=${var.docker_storagegw} do_project_prefix=${var.nc_prefix} do_project_suffix=${random_string.nc-random.result} instance_public_ip=${digitalocean_floating_ip.nc-ip.ip_address} do_storageaccessid=${var.do_storageaccessid} do_storagesecretkey=${var.do_storagesecretkey} web_port=${var.web_port} project_directory=${var.project_directory}' >> /var/log/nextcloud.log"
}
